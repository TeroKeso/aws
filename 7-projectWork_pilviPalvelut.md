...
3. Configure the bucket permission as below:
     1. Click the bucket name and on the top menu for the bucket, you will see **Permissions** tab. Click Permissions.
     2. Click **Edit** on the bucket policy and copy the following policy that allows retrieve permissions from the bucket. Remember to replace the bucket name with your own bucket name.
```
{
    "Version": "2012-10-17",
    "Statement": [
...
```
3. Upload an image to the bucket. You will get the Object URL. 
3. Use that Object URL to integrate an image in your Wordpress post after your WordPress website is installed.

> Note: (Optional)The ideal way of doing the above task is using some plugin in your Wrodpress website so that the images are automatically offloaded to the S3 bucket. If you wish, you can try the **Optimole** WordPress plugin. [How to offload WordPress media to S3 with Optimole](https://wpshout.com/wordpress-s3-guide/#optimole)
...
6. Connect to your Database Instance from the EC2 instance and create a database that you want to use for your WordPress website.
...
### Submission
You will submit a video (maximum 5 minutes long, voice over is not required) that includes the result after conducting the whole tasks. In the demonstration, you should include all tasks that you were able to implement that is your simple WordPress site is running that utilizes Amazon services as mentioned above. 
